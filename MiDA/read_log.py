...
class ReadLog:
    ...
    def get_time(self, sequence, max_trace, mean_trace):
        ...
        while i < len(sequence):
            ...
            while j < (len(sequence.iat[i, 0]) - 1):
                list_temp.append(
                    self.to_sec(self.time_format(sequence.iat[i, 0][j]) - self.time_format(sequence.iat[i, 0][0])))
    ...
    def mapping_cat(self, col, df_train, df_test, max_trace, mean_trace, fold):
        ...
        if col == 'timestamp':
            ...
            np.save("fold/" + self._eventlog + "/" + self._eventlog + '_' + col + '_' + str(fold) + "_train.npy",
                    view_train)  
            np.save("fold/" + self._eventlog + "/" + self._eventlog + '_' + col + '_' + str(fold) + "_test.npy",
                    view_test)  
            ...
            if is_numeric_dtype(df_train[col]):
                ...
                if col == 'activity':
                    ...
                    np.save(
                        "fold/" + self._eventlog + "/" + self._eventlog + '_' + col + '_' + str(fold) + "_train.npy",
                        view_train)  
                    np.save("fold/" + self._eventlog + "/" + self._eventlog + '_' + col + '_' + str(fold) + "_test.npy",
                            view_test)  
                    np.save("fold/" + self._eventlog + "/" + self._eventlog + '_' + col + '_' + str(fold) + "_info.npy",
                            len(mapping))  
                    np.save("fold/" + self._eventlog + "/" + self._eventlog + '_y_' + str(fold) + "_train.npy",
                            label_train)  
                    np.save("fold/" + self._eventlog + "/" + self._eventlog + '_y_' + str(fold) + "_test.npy",
                            label_test)  
    ...
    def readView(self):
        for fold in range(3):
            ...
            df_train = pd.read_csv(
                "fold/" + self._eventlog + "/" + self._eventlog + "_kfoldcv_" + str(fold) + "_train.csv",
                sep=',')  
            df_test = pd.read_csv(
                "fold/" + self._eventlog + "/" + self._eventlog + "_kfoldcv_" + str(fold) + "_test.csv",
                sep=',')  
