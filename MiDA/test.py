...
for f in range(3):
    ...
    for col in cat_view:
        list_cat_view_train.append(
            np.load("fold/" + eventlog + "/" + eventlog + "_" + col + "_" + str(f) + "_train.npy"))
    ...
    for col in num_view:
        list_num_view_train.append(
            np.load("fold/" + eventlog + "/" + eventlog + "_" + col + "_" + str(f) + "_train.npy", allow_pickle=True))

    list_num_view_test = []

    for col in num_view:
        list_num_view_test.append(
            np.load("fold/" + eventlog + "/" + eventlog + "_" + col + "_" + str(f) + "_test.npy", allow_pickle=True))
    ...
    precision, recall, fscore, _ = precision_recall_fscore_support(Y_test_int, preds_a, average='macro', pos_label=None)
