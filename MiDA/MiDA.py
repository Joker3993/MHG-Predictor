...
class MiDA:
    ...
    def get_model(self, cfg):
        ...
        opt = Nadam(learning_rate=cfg['learning_rate_init'], beta_1=0.9, beta_2=0.999, epsilon=1e-08,
    ...
    def fit_and_score(self, cfg):
        ...
        outfile2.write(str(score) + ";" + str(len(h.history['loss'])) + ";" + str(model.count_params()) + ";" +
                       str(end_time - start_time) + ";" + str(cfg['lstmsize1']) + ";" + str(cfg['lstmsize2']) + ";" +
                       str(cfg['batch_size']) + ";" + str(cfg['learning_rate_init']) + "\n")
    ...
    def smac_opt(self):
        ...
        for f in range(0,1):
            ...
            for col in self._cat_view:
                self.list_cat_view_train.append(
                    np.load("fold/" + self._eventlog + "/" + self._eventlog + "_" + col + "_" + str(f) + "_train.npy"))

            self.list_cat_view_test = []
            for col in self._cat_view:
                self.list_cat_view_test.append(
                    np.load("fold/" + self._eventlog + "/" + self._eventlog + "_" + col + "_" + str(f) + "_test.npy"))

            self.list_num_view_train = []
            for col in self._num_view:
                self.list_num_view_train.append(
                    np.load("fold/" + self._eventlog + "/" + self._eventlog + "_" + col + "_" + str(f) + "_train.npy",
                            allow_pickle=True))

            self.list_num_view_test = []
            for col in self._num_view:
                self.list_num_view_test.append(
                    np.load("fold/" + self._eventlog + "/" + self._eventlog + "_" + col + "_" + str(f) + "_test.npy",
                            allow_pickle=True))
